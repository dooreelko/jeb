...
def score_mc(model, article, classes, abstain=False, noun="topic"):
    """One score per option (label-token logits): the classes, then ABSTAIN if abstain is set.
    noun is what the options are called in the prompt. Works for up to 26 options."""
    ...
    prompt = model.chat(
        f"{intro(article, noun)}{opts}\n\nWhich {noun} is the most likely one?",
